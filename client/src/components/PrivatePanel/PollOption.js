...
function PollOption(props) {
  ...
  function addOption() {
    ...
    axios
      .post('/api/poll/options/add', newOne)
      .then(res => props.message(res.data))
      .catch(err => {
        poll.options.pop();
        props.message(err.response.data);
      }); //if update the options is fail, delete the new option,
  }
  ...
  return (
    <div className="polloption">
      <div style={{ display: 'flex', justifyContent: 'space-around' }}>
        <h3>{poll.subject}</h3>
        <button className="deleteBTN2" onClick={() => deletePoll(poll._id)}>
          X
        </button>
      </div>
      ...
      {addstatus ? (
        ...
        >
          {props => (
            <div style={props} className="addOption">
              <input
                type="text"
                name="addoption"
                id="addoption"
