...
class App extends Component {
  ...
  render() {
    const msgStyle = this.state.msgClosed
      ? { visibility: 'hidden' }
      : { display: 'flex' };
    return (
      <Router>
        ...
        {this.state.loginStatus ? (
          <button className="LogoutBTN" onClick={this.signout}>
            Sign Out
          </button>
        ) : null}
        <img
          src="https://kidstaskit.herokuapp.com/static/media/JunEBug.f67dd03a.png"
          className="logo"
          alt="logo"
        ></img>
        <div className="message" style={msgStyle}>
          ...
          <button
            style={{ width: '30px', marginTop: '10px', marginLeft: '20px' }}
            onClick={() => {
        ...
        <Switch>
                ...
                >
                  {this.state.loginStatus ? (
                    <Profile
                      ...
                      setPrivateView={this.setPrivateView}
                    />
                  ) : (
                    <Login status={this.getStatus} message={this.message} />
                  )}
                  {this.state.privateView ? (
                    <PrivatePanel user={this.state.user} />
                  ) : (
                    <Panel message={this.message} />
                  )}
                </div>
              );
            }}
          ...
          <Route
            ...
            exact
            render={props => {
              return <Register message={this.message} {...props} />;
            }}
          />
          {this.state.loginStatus ? (
            <Route
              ...
              exact
              render={props => {
                return <NewPoll message={this.message} {...props} />;
              }}
            />
          ) : (
            <Redirect to="/" />
